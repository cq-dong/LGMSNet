...
from network.Tinyunet import CMRF
...
class CMUNeXtBlock(nn.Module):
    def __init__(self, ch_in, ch_out, depth=1, k=3):
        ...
        self.block = nn.Sequential(
            *[nn.Sequential( CMRF(ch_in, ch_in)) for i in range(depth)]
        )
...
class fusion_conv_tiny(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(fusion_conv_tiny, self).__init__()
        self.conv = CMRF(ch_in, ch_out)
    def forward(self, x):
        x = self.conv(x)
        return x

class CMUNeXt_tiny1(nn.Module):
...
class CMUNeXt_tiny2(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = fusion_conv_tiny(ch_in=dims[3] * 2, ch_out=dims[3])
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = fusion_conv_tiny(ch_in=dims[2] * 2, ch_out=dims[2])
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = fusion_conv_tiny(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = fusion_conv_tiny(ch_in=dims[0] * 2, ch_out=dims[0])
        self.Conv_1x1 = nn.Conv2d(dims[0], num_classes, kernel_size=1, stride=1, padding=0)
