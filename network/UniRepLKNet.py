...
class UniRepLKNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.stem = nn.Sequential(
                nn.Conv2d(in_chans, stem_dim, kernel_size=(3, 3), padding=1, stride=1),
                nn.GELU(),
                nn.BatchNorm2d(stem_dim),
                Residual(nn.Sequential(
                    nn.Conv2d(stem_dim, stem_dim, kernel_size=(3, 3), groups=stem_dim, padding=1),
                    nn.GELU(),
                    nn.BatchNorm2d(stem_dim)
                )),
                nn.Conv2d(stem_dim, stem_dim, kernel_size=(1, 1)),
                nn.GELU(),
                nn.BatchNorm2d(stem_dim)
            )
        ...
        self.downsample_layers.append(
            nn.Sequential(
                nn.Conv2d(stem_dim, dims[0], kernel_size=3, stride=1, padding=1),
                nn.MaxPool2d((2, 2))
                ))
...
class Utr(nn.Module):
    def __init__(self,
                 ...
                 out_channel=1,
                 dims=(40, 80, 160, 320)):
        super(Utr, self).__init__()

        self.encoder = UniRepLKNet(in_chans=in_channel, depths=UniRepLKNet_A_F_P_depths, dims=(40, 80, 160, 320))
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.fusion_conv4 = conv_block(ch_in=dims[2] * 2, ch_out=dims[2])
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.fusion_conv3 = conv_block(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.fusion_conv2 = conv_block(ch_in=dims[0] * 2, ch_out=dims[0])
        self.Up1 = up_conv(ch_in=dims[0], ch_out=16)
        self.fusion_conv1 = conv_block(ch_in=16 * 2, ch_out=16)
        self.head = nn.Conv2d(16, out_channel, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        x = self.Up4(x4)
        x = self.fusion_conv4(torch.cat((x, x3), dim=1))
        x = self.Up3(x)
        x = self.fusion_conv3(torch.cat((x, x2), dim=1))
        x = self.Up2(x)
        x = self.fusion_conv2(torch.cat((x, x1), dim=1))
        x = self.Up1(x)
        x = self.fusion_conv1(torch.cat((x, x0), dim=1))
        x = self.head(x)
        return x
