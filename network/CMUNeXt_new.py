...
import sys
sys.path.append("/data/chengqidong/project/segmentation/CMUNeXt/network")
...
from network.Mobilevit import MobileViTBlocktem_noffn,MobileViTBlocktem_CIT,MobileViTBlocktem_CT_mask,MobileViTBlocktem_CT_2,MobileViTBlocktem_CT_3
from network.SCSA import PCSA
...
class Unetconv_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(Unetconv_block, self).__init__()
...
class CMUNeXtBlock_MK_resiual22(nn.Module):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(self, ch_in, ch_out,
                group_kernel_sizes= [3, 5, 7, 9],
                ): # ch_in 需要是4的倍数
        super().__init__()
...
class CMUNeXtBlock_MK_resiual2_new(nn.Module):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(self, ch_in, ch_out,
                group_kernel_sizes= [3, 5, 7, 9],
                ): # ch_in 需要是4的倍数
        super().__init__()
        ...
        self.conv_1x1_bn=conv_1x1_bn(ch_in,self.group_chans)
        self.conv_nxn_bn=conv_nxn_bn(self.group_chans,ch_in)
        self.pcsa=PCSA(dim=ch_in,head_num=8)
    ...
    def forward(self, x):
        ...
        # 然后用1*1交互一下channel，这块也可以用注意力什么的想一想
        x_attn = self.conv_1x1_bn(self.pcsa(x_attn))
        x=self.conv_nxn_bn(x_attn)
        return x+xclone
...
class CMUNeXtBlock_MK2_resiual3(nn.Module):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(self, ch_in, ch_out,
                group_kernel_sizes= [3, 5, 7, 9],
                ): # ch_in 需要是4的倍数
        super().__init__()
        ...
        self.conv_1x1_bn=conv_1x1_bn(ch_in+self.group_chans,ch_in)
        self.conv_nxn_bn=conv_nxn_bn(ch_in,self.group_chans)
        self.conv_nxn_bn2=conv_nxn_bn(ch_in,ch_in)
    ...
    def forward(self, x):
        ...
        # x_attn = self.conv_nxn_bn(x_attn)
        x=self.conv_1x1_bn(torch.cat((x_attn,self.conv_nxn_bn(x_attn)),dim=1))
        x=self.conv_nxn_bn2(x)
        return x+xclone


class CMUNeXtBlock_MK2_resiual4(nn.Module):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(self, ch_in, ch_out,
                group_kernel_sizes= [3, 5, 7, 9],
                ): # ch_in 需要是4的倍数
        super().__init__()
        ...
        self.global_dwc_l = nn.Conv2d(self.group_chans, self.group_chans, kernel_size=group_kernel_sizes[3], 
                padding=group_kernel_sizes[3] // 2, groups=self.group_chans)
        self.conv_1x1_bn=conv_1x1_bn(ch_in+self.group_chans,self.group_chans)
        self.conv_nxn_bn=conv_nxn_bn(ch_in,self.group_chans)
        self.conv_nxn_bn2=conv_nxn_bn(self.group_chans,ch_in)
...
class fusion_convMK1(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out * 4, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out * 4),
            CMUNeXtBlock_MK2_resiual2(ch_out * 4, ch_out * 4,group_kernel_sizes= [3, 5, 7, 9]),
            nn.Conv2d(ch_out * 4, ch_out, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out)
        )
...
class fusion_convMK2(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.conv = nn.Sequential(
            ...
            CMUNeXtBlock_MK2_resiual2(ch_out * 2, ch_out * 2,group_kernel_sizes= [3, 5, 7, 9]),
...
class fusion_convMK3(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out * 4, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out * 4),
            CMUNeXtBlock_MK2_resiual2(ch_out * 4, ch_out * 4,group_kernel_sizes= [1,3,5,7]),
            nn.Conv2d(ch_out * 4, ch_out, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out)
        )
...
class fusion_convMK4(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out * 2, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out * 2),
            CMUNeXtBlock_MK2_resiual2(ch_out * 2, ch_out * 2,group_kernel_sizes= [1,3,5,7]),
            nn.Conv2d(ch_out * 2, ch_out, kernel_size=(1, 1)),
            nn.GELU(),
            nn.BatchNorm2d(ch_out)
        )
...
Block_dict={
    ...
    "CMUNeXtBlock_MK2_resiual3":CMUNeXtBlock_MK2_resiual3,
    "CMUNeXtBlock_MK2_resiual4":CMUNeXtBlock_MK2_resiual4,
    "CMUNeXtBlock_MK_resiual2_new":CMUNeXtBlock_MK_resiual2_new,
...
class MKtinyvit4_CT_new1(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = nn.Sequential(MobileViTBlocktem_CT(dims[3]* 2, 1,dims[3],kernel_size=3, spilt_list=spilt_list[2], patch_size=(2,2), mlp_dim=int(dims[3] * 2)),CMUNeXtBlockmodel(ch_in=dims[3], ch_out=dims[3]))
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = nn.Sequential(fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2]),MobileViTBlocktem_CT(dims[2], 1,dims[2],kernel_size=3,spilt_list=spilt_list[3], patch_size=(2,2), mlp_dim=int(dims[2] * 2)),CMUNeXtBlockmodel(ch_in=dims[2], ch_out=dims[2]) )
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = fusion_conv(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = fusion_conv(ch_in=dims[0] * 2, ch_out=dims[0])
...
class MKtinyvit4_CT_new2(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.Up_conv4 = nn.Sequential(fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2]),MobileViTBlocktem_CT(dims[2], 1,dims[2],kernel_size=3,spilt_list=spilt_list[3], patch_size=(2,2), mlp_dim=int(dims[2] * 2)),CMUNeXtBlockmodel(ch_in=dims[2], ch_out=dims[2]))
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = nn.Sequential(fusion_conv(ch_in=dims[1] * 2, ch_out=dims[1]),CMUNeXtBlockmodel(ch_in=dims[1], ch_out=dims[1]))
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = nn.Sequential(fusion_conv(ch_in=dims[0] * 2, ch_out=dims[0]),CMUNeXtBlockmodel(ch_in=dims[0], ch_out=dims[0]))
        self.Conv_1x1 = nn.Conv2d(dims[0], num_classes, kernel_size=1, stride=1, padding=0)
...
class DoublePool(nn.Module):
    def __init__(self, ch_in,ch_out,kernel_size=2, stride=2):
        ...
        self.conv11  = conv_1x1_bn(ch_in*2,ch_out)

    def forward(self, x):
        max_out = self.maxpool(x)   # 最大池化分支
        avg_out = self.avgpool(x)   # 平均池化分支
        # 在通道维度拼接（假设输入x的通道为C，拼接后通道为2C）
        out = self.conv11(torch.cat([max_out, avg_out], dim=1))
        return out
class MKtinyvit4_down_CT(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.pool1 = DoublePool(dims[0],dims[0],kernel_size=2, stride=2)
        self.pool2 = DoublePool(dims[1],dims[1],kernel_size=2, stride=2)
        self.pool3 = DoublePool(dims[2],dims[2],kernel_size=2, stride=2)
        self.pool4 = DoublePool(dims[3],dims[3],kernel_size=2, stride=2)
...
class MKtinyvit4_CT_7(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
...
class MKtinyvit4_CT_deeps(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.d5_output_layer=nn.Sequential(conv_nxn_bn(dims[3],dims[3]),conv_1x1_bn(dims[3],num_classes))
        self.d4_output_layer=nn.Sequential(conv_nxn_bn(dims[2],dims[2]),conv_1x1_bn(dims[2],num_classes))
        self.d3_output_layer=nn.Sequential(conv_nxn_bn(dims[1],dims[1]),conv_1x1_bn(dims[1],num_classes))
        # self.d2_output_layer=nn.Sequential(conv_nxn_bn(dims[3],dims[3]),conv_1x1_bn(dims[3],num_classes))
        # self.d1_output_layer=nn.Sequential(conv_nxn_bn(dims[3],dims[3]),conv_1x1_bn(dims[3],num_classes))
    ...
    def forward(self, x):
        ...
        d5_0 = self.d5_output_layer(d5)

        d4 = self.Up4(d5)
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4)

        d4_0 = self.d4_output_layer(d4)
...
class MKtinyvit4_CT_Unetblock_Ablation1(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.stem = conv_block(ch_in=input_channel, ch_out=dims[0])
        self.encoder1 = nn.Sequential(Unetconv_block(dims[0],dims[0]))
        self.encoder2 = nn.Sequential(Unetconv_block(dims[0],dims[1]))
        self.encoder3 = nn.Sequential(Unetconv_block(dims[1],dims[2]))
        self.encoder4 = nn.Sequential(MobileViTBlocktem_CT(dims[2], 1,dims[3],kernel_size=3,spilt_list=spilt_list[0], patch_size=(2,2), mlp_dim=int(dims[3] * 2)))
        self.encoder5 = nn.Sequential(MobileViTBlocktem_CT(dims[3], 1,dims[4],kernel_size=3,spilt_list=spilt_list[1],  patch_size=(2,2), mlp_dim=int(dims[4] * 2)))
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
...
class MKtinyvit4_CT_Unetblock_Ablation2(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
...
class MKtinyvit4_CT_S1(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.encoder2 = nn.Sequential(*[CMUNeXtBlockmodel(ch_in=dims[0], ch_out=dims[1]) for i in range(depths[1])],conv_1x1_bn(dims[0],dims[1]))
        self.encoder3 = nn.Sequential(*[CMUNeXtBlockmodel(ch_in=dims[1], ch_out=dims[2]) for i in range(depths[2])],conv_1x1_bn(dims[1],dims[2]))
        self.encoder4 = nn.Sequential(MobileViTBlocktem_CT_2(dims[2], 1,dims[3],kernel_size=2,spilt_list=spilt_list[0], patch_size=(2,2), mlp_dim=int(dims[3] * 2)))
        self.encoder5 = nn.Sequential(MobileViTBlocktem_CT_2(dims[3], 1,dims[4],kernel_size=2,spilt_list=spilt_list[1],  patch_size=(2,2), mlp_dim=int(dims[4] * 2)))
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktem_CT_2(dims[3]* 2, 1,dims[3],kernel_size=2, spilt_list=spilt_list[2], patch_size=(2,2), mlp_dim=int(dims[3] * 2))
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = nn.Sequential(fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2]),MobileViTBlocktem_CT_2(dims[2], 1,dims[2],kernel_size=2,spilt_list=spilt_list[3], patch_size=(2,2), mlp_dim=int(dims[2] * 2)))
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = fusion_conv(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = fusion_conv(ch_in=dims[0] * 2, ch_out=dims[0])
...
class MKtinyvit4_CT_A(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
        ...
        self.encoder2 = nn.Sequential(*[CMUNeXtBlockmodel(ch_in=dims[0], ch_out=dims[1]) for i in range(depths[1])],conv_1x1_bn(dims[0],dims[1]))
        self.encoder3 = nn.Sequential(*[CMUNeXtBlockmodel(ch_in=dims[1], ch_out=dims[2]) for i in range(depths[2])],conv_1x1_bn(dims[1],dims[2]))
        self.encoder4 = nn.Sequential(MobileViTBlocktem_CT_3(dims[2], 1,dims[3],kernel_size=3,spilt_list=spilt_list[0], patch_size=(2,2), mlp_dim=int(dims[3] * 2)))
        self.encoder5 = nn.Sequential(MobileViTBlocktem_CT_3(dims[3], 1,dims[4],kernel_size=3,spilt_list=spilt_list[1],  patch_size=(2,2), mlp_dim=int(dims[4] * 2)))
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktem_CT_3(dims[3]* 2, 1,dims[3],kernel_size=3, spilt_list=spilt_list[2], patch_size=(2,2), mlp_dim=int(dims[3] * 2))
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = nn.Sequential(fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2]),MobileViTBlocktem_CT_3(dims[2], 1,dims[2],kernel_size=3,spilt_list=spilt_list[3], patch_size=(2,2), mlp_dim=int(dims[2] * 2)))
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = fusion_conv(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = fusion_conv(ch_in=dims[0] * 2, ch_out=dims[0])
...
fusion_conv_DICT={"fusion_convMK1":fusion_convMK1,"fusion_convMK2":fusion_convMK2,
                  "fusion_convMK3":fusion_convMK3,"fusion_convMK4":fusion_convMK4,
                  }

class AllMKtinyvit4_CT(nn.Module):
    def __init__(self, model="CMUNeXtBlock_MK_resiual",fusion="fusion_convMK1",spilt_list=[[96,32],[96,32],[96,32],[48,16]],input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
            num_classes: output channel.
        ...
        self.encoder5 = nn.Sequential(MobileViTBlocktem_CT(dims[3], 1,dims[4],kernel_size=3,spilt_list=spilt_list[1],  patch_size=(2,2), mlp_dim=int(dims[4] * 2)))
        # Decoder
        fusion_convMK = fusion_conv_DICT[fusion]
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktem_CT(dims[3]* 2, 1,dims[3],kernel_size=3, spilt_list=spilt_list[2], patch_size=(2,2), mlp_dim=int(dims[3] * 2))
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = nn.Sequential(fusion_convMK(ch_in=dims[2] * 2, ch_out=dims[2]),MobileViTBlocktem_CT(dims[2], 1,dims[2],kernel_size=3,spilt_list=spilt_list[3], patch_size=(2,2), mlp_dim=int(dims[2] * 2)))
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = fusion_convMK(ch_in=dims[1] * 2, ch_out=dims[1])
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = fusion_convMK(ch_in=dims[0] * 2, ch_out=dims[0])
        self.Conv_1x1 = nn.Conv2d(dims[0], num_classes, kernel_size=1, stride=1, padding=0)
